...
if run:
    with st.spinner("Running optimization…"):
        results, summary = run_optimization(
            ...
            w_critical=w_critical, w_mild=w_mild, w_safe=w_safe,
    ...
    <div class="kpi-grid">
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-delta kpi-gray">8-quarter horizon</div>
        </div>
    ...
    st.markdown(
        ...
        f'<strong>Horizon: 8 quarters (2 years)</strong> · '
    ...
    # ── LTV Curve ──────────────────────────────────────────────────────────
    st.markdown("#### 📈 Platform Value Accumulation — 8-Quarter Horizon")
    st.caption(
        "Cumulative platform value per market over 8 quarters. "
        "Q1 = direct NPM1. Q2–Q8 = compound retention (35%/qtr) discounted at 2.5%/qtr "
    ...
    for _, row in funded.iterrows():
        ...
        for t in range(1, N_QUARTERS + 1):
            if t == 1:
                ...
                pv_t = gb_delta * margin * (RETENTION ** (t - 1)) / ((1 + DISCOUNT) ** (t - 1))
                cumulative += pv_t
